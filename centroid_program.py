...
def getUncertainty(actualX, actualY, selectedData, sumIntensities):  #based on std dev of mean
    leftboundY = centerY - side/2
    leftboundX = centerX - side/2
    N = sumIntensities
    uncertaintyNumeratorX = 0
    sumXcol = 0
    uncertaintyNumeratorY = 0
    sumYcol = 0
    sumVarX = 0
    sumVarY = 0
    selectedDataFlipped = selectedData.T #transposed matrix to intuitively iterate through rows and cols
    for i in range(selectedDataFlipped.shape[0]):
        sumXcol = 0
        for row in range(selectedDataFlipped.shape[1]):
            sumXcol += selectedDataFlipped[i, row]
        sumVarX += ((actualX - (leftboundX + i)) ** 2) * sumXcol
    sigmaX = math.sqrt(sumVarX/(N*(N-1)))
    print "Uncertainty X: ", sigmaX
    
    for i in range(selectedDataFlipped.shape[1]):
        sumYcol = 0
        for row in range(selectedDataFlipped.shape[0]):
            sumYcol += selectedDataFlipped[row, i]
        sumVarY += ((actualY - (leftboundY + i)) ** 2) * sumYcol
    sigmaY = math.sqrt(sumVarY/(N*(N-1)))
    print "Uncertainty Y: ", sigmaY
